                            ...
                            <tbody>
                                 @forelse ($postres as $postre )
                                    <tr class=" dark:bg-dark-1 {{ $loop->index % 2> 0 ? 'bg-gray-200' : '' }}">
                                        <td>
                                            <img src="{{ $postre->img }}" data-action="zoom" alt="img-category" width="100">
                                        </td>

                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $postre->descripcion }}</h6>
                                        </td>
                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $postre->precio }}</h6>
                                        </td>
                                        ...
                                        <td class="dark:border-dark-5 text-center">
                                            <div class="d-flex justify-content-center">
                                                {{-- @if ($category->products->count() < 1) --}}
                                                    ...
                                                    onclick="destroy('postres','Destroy', {{ $postre->id }})"
                                                    type="button">
                                                        <i class=" fas fa-trash f-2x"></i>
                                                    </button>
                                                {{-- @endif --}}
                                                <button class="btn btn-warning text-white border-0 ml-3"
                                                    wire:click.prevent="Edit({{ $postre->id }})"
                                                    type="button">
                                ...
                                @empty
                                    <tr class="bg-gray-200 dark:bg-dark-1">
                                        <td colspan="2">
                                            <h6 class="text-center">    NO HAY POSTRES REGISTRADAS </h6>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
